...
def add_product_without_timing(product_id, name, category, price, quantity):
    """
    Adds a product to the inventory and inserts the price into the min-heap and AVL tree.
    This version is used for initialization and doesn't track the time taken for performance.
    """
    if product_id in inventory:
        print(f"Error: Product with ID {product_id} already exists.")
        return
    ...
    # Add the product to the inventory (hash table)
    inventory[product_id] = product
    
    # Add the product's price and ID to the heap for price tracking
    add_product_to_heap(product_id, price)

    # Insert the product into the AVL tree for price-range queries
    global avl_root  # Ensure we're modifying the global avl_root
